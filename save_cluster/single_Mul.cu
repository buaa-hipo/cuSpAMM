...
__global__ void unroll_get_Fnorm(const float* __restrict__ A,float *A_normmap,int m,int n){
    int id = blockIdx.x * blockDim.x + threadIdx.x;
    int kId = blockIdx.x;//kernel
    int thId = threadIdx.x;
    __shared__ float sdata[LoNum*LoNum];
    ...
    int myFinalRow = myBlockRow*LoNum+myThreadRow;
    int myFinalCol = myBlockCol*LoNum+myThreadCol;
...
int main(int argc, char **argv){
    int device_row_offset=T/LoNum/2;
    //测试part是否太大
    if(T/LoNum/2/PART<=0){
    ...
    //统一内存h_A,h_B
    float *h_A,*h_B;
    cudaMallocManaged((void **)&h_A, sizeof(float)*T*T);
    cudaMallocManaged((void **)&h_B, sizeof(float)*T*T);
    //给A,B赋值
    ...
    for(int i=0;i<2;i++){
        ...
        cudaMalloc((void **)&plan[i].A_normmap, sizeof(float)*(T/LoNum)*(T/LoNum));
        cudaMalloc((void **)&plan[i].B_normmap, sizeof(float)*(T/LoNum)*(T/LoNum));
    ...
    for(int i=0;i<TESTTIME;i++){
        ...
        for(int device=0;device<2;device++){
            ...
            unroll_get_Fnorm<<<A_blocks,F_threads/8,0,plan[device].stream>>>(plan[device].h_A,plan[device].A_normmap,M,K);
            unroll_get_Fnorm<<<B_blocks,F_threads/8,0,plan[device].stream>>>(plan[device].h_B,plan[device].B_normmap,M,K);
            ...
            for(int p=0;p<PART;p++){
                //注意，算的单位是block，偏移的单位也是block
                get_C_Threads1Element_Mul<<<C_blocks/2/PART,C_threads,0,plan[device].stream>>>(plan[device].h_A,plan[device].A_normmap,plan[device].h_B,plan[device].B_normmap,plan[device].h_C,device*device_row_offset+p*T/LoNum/2/PART);
                cudaStreamSynchronize(plan[device].stream);
            }
    ...
    if(CHECK) {
        // //整合最终C的结果
        float* result_C;//不能用局部变量
        cudaMallocHost((void **)&result_C, sizeof(float)*T*T);
        float* t_C=plan[0].h_C;
        for(int i=0;i<T;i++){
            if(i==T/2) t_C=plan[1].h_C;
            for(int j=0;j<T;j++){
                result_C[i*T+j]=t_C[i*T+j];
            }
        }

        check(h_A,h_B,result_C);
        // check_simple_matrix_mul(h_A,h_B,result_C);

        //取0号的normmap验证
        // float h_Amap[M*K/(LoNum*LoNum)],h_Bmap[K*N/(LoNum*LoNum)];
        // cudaMemcpy(&h_Amap, plan[0].A_normmap, sizeof(float)*(T/LoNum)*(T/LoNum), cudaMemcpyDeviceToHost);
        // cudaMemcpy(&h_Bmap, plan[0].B_normmap, sizeof(float)*(T/LoNum)*(T/LoNum), cudaMemcpyDeviceToHost);
