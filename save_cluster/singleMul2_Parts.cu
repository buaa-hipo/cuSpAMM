...
int main(int argc, char **argv){
    ...
    for(int i=0;i<TESTTIME;i++){
        ...
        for(int device=0;device<2;device++){
            ...
            for(int p=0;p<PART;p++){
                unroll_get_Fnorm<<<A_blocks/PART,F_threads/8,0,plan[device].stream>>>(plan[device].h_A,plan[device].A_normmap,M,K,device*(T/LoNum/2)+p*partBlockOffset);

                cudaStreamSynchronize(plan[device].stream);

                get_C_Threads1Element_Mul<<<C_blocks/2/PART,C_threads,0,plan[device].stream>>>(plan[device].h_A,plan[device].A_normmap,plan[device].h_B,plan[device].B_normmap,plan[device].h_C,device*(T/LoNum/2)+p*(partBlockOffset/2));
            }
        }
    ...
    if(CHECK) {
        ...
        // for(int i=0;i<T;i++){
        //     for(int j=0;j<T;j++){
        //         result_C[i*T+j]=plan[i/(T/2)].h_C[i*T+j];
        //     }
        // }

        // check(h_A,h_B,result_C);
        // // MATRIXSHOW21D(result_C,T,T);
        // // // check_simple_matrix_mul(h_A,h_B,result_C);

        // //取0号的normmap验证
        // float h_Amap[(T/LoNum)*(T/LoNum)],h_Bmap[(T/LoNum)*(T/LoNum)];
