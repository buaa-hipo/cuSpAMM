...
double run_cublas_time(mytype* A,mytype* B){
    ...
    for(int i=0;i<TESTTIME;i++){
        ...
        elapsed /= 1000.0f;
        if(i>=WARMUP+1) sum += elapsed;
    }
    // printf("cuBLAS平均执行时间=%fs\n",sum/(TESTTIME-WARMUP));
    return (double)(sum/(TESTTIME-WARMUP));
}
...
//计算有效块乘的个数
float countValid(float* A_normmap,float* B_normmap){
    printf("\n***统计SpAMM有效比***\n");
    int count=0;
    float norm,ave=0;
    int validC[M/LoNum][N/LoNum];
    ...
    int totalNum=(M/LoNum)*(N/LoNum)*(K/LoNum);
    // CHECKEQ6(C,gpu_C,M,N);
    printf("SpAMM真实乘法数量=%d, 稠密乘乘法数量=%d, 有效比=%f%\n",count,totalNum,(float)count/totalNum*100);
    return (float)count/totalNum;
...
void truncation(float* M, float* ORI,float flag){
    ...
    printf("spares matrix 截断的元素个数=%d 非零值个数=%d 非零比=%f%%\n",del,T*T-del,(float)(T*T-del)/(T*T)*100);
}
