...
int main(int argc, char **argv){
    ...
    if(CHECK) {
        ...
        float validrate=countValid(h_Amap,plan[0].B_normmap);

        //计算flops,乘加，所以flops数为MNK*2
        double flops=((double)(M/1024)*(N/1024)*(K/1024)*2*validrate)/spammtime;
        printf("cuSpAMM flops=%f*1e40\n",flops/1024);
